...
public class ImageHandler : MonoBehaviour{
    ...
    protected IEnumerator pulse_value(string image_id, string value, int pulses = 1, float time = 0.35f){
        int count = pulses;
        while (count > 0){
            yield return StartCoroutine(lerp_value(image_id, value, 1f, 0f, time));
            --count;
            yield return null;
        }
    ...
    protected IEnumerator lerp_value(string image_id, string value, float start, float end, float time, Action callback = null) {
    ...
    protected void set_value(string value_id, float value){
        foreach(string image in images.Keys)
            set_value(image, value_id, value);
    }
    protected void set_value(string sprite_id, string value_id, float value) => images[sprite_id].material.SetFloat(value_id, value);

    public void enable_image(string id, bool enabled) => images[id].enabled = enabled;
    public void enable_image(bool enabled){
        foreach(Image image in images.Values)
